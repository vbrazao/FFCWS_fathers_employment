...
Each of the following three graphs has the same structure. The top panel shows the effect size (which can be interpreted as percentage point differences) with confidence interval for each specification, and is colored according to significance. The bottom panel shows which combination of analytical choices produced the corresponding effect size above.

First, we plot the effect of unemployment on White IPV perpetration (Figure 1), then the effect of unemployment on Black IPV perpetration (Figure 2), and finally we subtract the second from the first to show the interaction (Figure 3).
...
Figure 3 shows that a most specifications result in a "negative", albeit not significant, interaction effect, such that the difference in IPV between employed and unemployed men is more negative for White men as compared to Black men. Here again we see that models using binarized IPV show the starkest negative interaction effect. The other specification options don't seem to have a systematic effect on the estimated interaction.
...
Next we home in on each outcome in turn (total, emotional, controlling, and physical IPV), without arranging by size of estimate. In each figure, the top plot shows the three effects (Top: effect of unemployment among Black men, Middle: effect of unemployment among White men, Bottom: interaction effect) while the bottom plot describes the corresponding specifications.
