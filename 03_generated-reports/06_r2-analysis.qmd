...
# for our survey models, we want modelsummary() to retrieve the AIC value that is stored in the model object
# instead of trying to calculate it as there's some sort of bug there (still to ask on StackOverflow about this)
AIC.svrepglm <- function(model){
  AIC <- model$aic
